...
final class WebService(geoLocationPersistence: GeoLocationPersistence)(
    implicit executionContext: ExecutionContextExecutor) {

  private val totallyMissingHandler = RejectionHandler
    .newBuilder()
    .handleNotFound {
      complete(
  ...
  private val geoLocationController = new GeoLocationController(
    geoLocationPersistence)
...
object Routes {
  ...
  def getGeoLocationRoute(controller: GeoLocationController)(
      ...
      rejectEmptyResponse {
        redirectToNoTrailingSlashIfPresent(StatusCodes.MovedPermanently) {
          path("locations") {
            pathEnd {
              get {
                parameters(('pageNumber.as[Long].?, 'pageSize.as[Long].?))
                  .as(PageParams) { pageParams =>
