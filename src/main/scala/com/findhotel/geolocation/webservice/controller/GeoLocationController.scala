...
class GeoLocationController(persistence: GeoLocationPersistence) {
  ...
  def getLocationEntities(pageParams:PageParams)(implicit ec: ExecutionContextExecutor): Route = {

    val items: Future[Seq[Geolocation]] = persistence.getLocations(pageParams.skip, pageParams.limit)
    onSuccess(items)(complete(_))
  }

  def getLocationEntity(ipAddress: String)(implicit ec: ExecutionContextExecutor): Route = {
