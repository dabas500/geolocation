...
class H2GeoLocationPersistence(h2Database: Database)(implicit ec: ExecutionContext, timeout:Duration) extends GeoLocationPersistence{
  ...
  private def fromBbReprGeoLocation(rows: Seq[GeoLocationDBRepr]): Seq[Geolocation] =
    rows.map(result=>
  ...
  override def getLocation(ipAddress: String): Future[Seq[Geolocation]] = {
    val getLocationByIpAddress = geoLocationTable.filter(data => data.ipAddress === ipAddress).result
  ...
  override def addGeoLocation(data: Geolocation): Future[Int] = {
    val addGeoLocationQuery = geoLocationTable.insertOrUpdate(toDBReprGeoLocation(data))
    h2Database.run(addGeoLocationQuery)
  }

  override def getLocations(offset: Long, limit: Long): Future[Seq[Geolocation]] ={
    val query = geoLocationTable.filter(row=> row.locationId >= offset && row.locationId <= (offset+limit)).result
    h2Database.run(query).map(fromBbReprGeoLocation(_))
  }


  override def addAllGeoLocations(rows: Seq[Geolocation]): Future[Int] = {
    val toBeInserted = rows.map { row => TableQuery[GeoLocationTable].insertOrUpdate(toDBReprGeoLocation(row))}
